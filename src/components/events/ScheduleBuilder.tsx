...
import { Plus, X, MapPin, CalendarCheck } from "lucide-react";
...
export function ScheduleBuilder({ value, onChange, eventId }: ScheduleBuilderProps) {
  ...
  // Track booking intent per schedule index during event creation.
  // University buildings default to true (auto-booked), non-university default to false.
  const [bookingIntent, setBookingIntent] = useState<Record<number, boolean>>({});
        ...
        <Card key={index}>
          <CardContent>
            <div className="space-y-3">
              ...
              {/* Row 2: Building + Room + Manage Booking */}
              <div className="grid grid-cols-[1fr_1fr_auto] gap-3 items-end">
                ...
                {(() => {
                  ...
                  // University defaults to booked (true), non-university defaults to not booked (false)
                  const isBooked = bookingIntent[index] ?? isUni;
                  ...
                  if (isBooked) {
                    return (
                      <Button
                        ...
                        onClick={() => {
                          setBookingIntent((prev) => ({ ...prev, [index]: false }));
                  ...
                  return (
                    <Button
                      ...
                      onClick={() => {
                        setBookingIntent((prev) => ({ ...prev, [index]: true }));
